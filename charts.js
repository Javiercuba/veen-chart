looker.plugins.visualizations.add({
  ...
  options: {
    ...
    linkTarget: {
      ...
      type: "string",
      display: "select",
      values: [{ "New Tab": "New Tab" }, { "Same Tab": "Same Tab" }],
      default: "New Tab",
      section: "Linking",
      order: 1,
    },
  ...
  update: function (data, element, config, queryResponse) {
    ...
    for (var row of data) {
      ...
      var measure_object = row[queryResponse.fields.measures[0].name];
      var measure_value = measure_object.value;
    ...
    if (!d3.selectAll("#venn > svg").empty()) {
      d3.selectAll("svg").remove();
    }

    //Draw the initial set
    var colorArray = [
      config.circleColors[1],
      config.circleColors[2],
      config.circleColors[0],
      config.circleColors[3],
      config.circleColors[4],
      config.circleColors[5],
      config.circleColors[6],
      config.circleColors[7],
    ...
    if (config.sublabels) {
      ...
      layout.enter
        .append("text")
        .text(function (d) {
          return d.size.toLocaleString("en");
        })
        .attr("class", "sublabel")
    ...
    if (config.tooltip) {
      ...
      div
        ...
        .on("mouseover", function (d, i) {
          ...
          if (config.isLink) {
            var pointer = d3.select(this).style("cursor", "pointer");
          }
        ...
        .on("mouseout", function (d, i) {
          ...
          selection
            ...
            .style(
              "fill-opacity",
              d.sets.length == 1 ? config.circleOpacity : 0
            )
            .style("stroke-opacity", 0);
        });
    }
...
centers the diagram in the available space at the same time */
  ...
  function VennDiagram(colorArray, factor) {
    var width = 600 * factor,
      height = 350 * factor,
      padding = 15,
      duration = 1000,
      orientation = Math.PI / 2,
